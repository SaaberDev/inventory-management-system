    ...
    <div class="card-body table-responsive p-0">
        <table style="width: 100%" class="table table-hover text-sm-center">
            ...
            @forelse($purchases as $purchase)
                <tr class='clickable-row' data-href='{{ route('admin.purchase.self.show', $purchase->code) }}'>
                    <td>{{ $purchase->purchase_date }}</td>
                    <td>
                        <a href="{{ route('admin.purchase.self.show', $purchase->code) }}" class="">{{ $purchase->code }}</a>
                    </td>
                    <td>{{ $purchase->suppliers->name }}</td>
                    <td>{{ $purchase->total_paid }} Tk</td>
                    <td>{{ $purchase->total_due }} Tk</td>
                    <td>{{ $purchase->grand_total }} Tk</td>
                    <td>
                        <span class="badge {{ ($purchase->payment_status == 'paid' ? 'badge-info' : '') ?: ($purchase->payment_status == 'due' ? 'badge-warning' : '') }}">
                    ...
                    <td>
                        <div class="d-flex justify-content-center">
                            @can('purchase.edit')
                                <a href="{{ route('admin.purchase.self.edit', $purchase->id) }}"
                               class="btn p-0 mr-3"
                            >
                            ...
                            @endcan
                            @can('purchase.delete')
                                <a href="{{ route('admin.purchase.self.delete', $purchase->id) }}"
