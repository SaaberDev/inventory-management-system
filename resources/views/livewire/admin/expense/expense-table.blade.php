    ...
    <div class="card-body table-responsive p-0">
        <table style="width: 100%" class="table table-hover text-sm-center">
            ...
            @forelse($expense as $expenses)
                <tr wire:key="$expense-{{ $loop->index }}">
                    <td>{{ $expenses->expenses_date }}</td>
                    <td>{{ $expenses->reason }}</td>
                    <td>
                        {{ $expenses->ExpenseCategory->name }}
                    </td>
                    <td>
                        {{ $expenses->ExpenseCategory->desc }}
                    ...
                    <td>
                        ...
                        @can('expense.edit')
                            <a href="{{ route('admin.expense.self.edit', $expenses->id) }}"
                               class="btn p-0 mr-3"
                            >
                        ...
                        @endcan
                        @can('expense.delete')
                            <a href="{{ route('admin.expense.self.delete', $expenses->id) }}"
            ...
            @empty
                <tr>
                    <td colspan="7">No Expense Available</td>
                </tr>
            @endforelse
            </tbody>
        </table>
