...
@section('content')
    <div class="card">
        ...
        <!-- /.card-header -->
        <div class="card-body">
            <form action="{{ route('admin.product.self.update', $product->id) }}"
                  method="POST"
                  autocomplete="off"
                            ...
                            >
                                ...
                                @foreach($product_types as $product_type)
                                    <option value="{{ $product_type->id }}"
                                        {{ $product_type->id == old('product_type', $product->product_type_id) ? 'selected' : '' }}
                                    >
                                        {{ $product_type->name }}
                                    </option>
                                @endforeach
                            ...
                            >
                                ...
                                @foreach($units as $unit)
                                    <option value="{{ $unit->id }}"
                                        {{ $unit->id == old('unit', $product->unit_id) ? 'selected' : '' }}
                                    >
                                        {{ $unit->name }}
                                    </option>
                                @endforeach
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text"
                                   ...
                                   value="{{ old('stock_alert_limit', $product->stock_alert_limit) }}"
