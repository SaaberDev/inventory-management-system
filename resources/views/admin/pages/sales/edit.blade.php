...
@section('add-new-button')
    <a type="button" href="{{ route('admin.sales.self.index') }}" class="btn btn-info btn-sm float-sm-right">View List</a>
@endsection
...
@section('content')
    <div class="card">
        ...
        <!-- /.card-header -->
        <div class="card-body">
            <form action="{{ route('admin.sales.self.update', $sale->id) }}"
                  method="POST"
                  autocomplete="off"
            ...
            >
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label for="">Sales Date<span class="text-danger"> *</span></label>
                            <input type="text"
                                   value="{{ old('sales_date', $sale->sale_date ) }}"
                                   name="sales_date"
                                   class="form-control sales_date {{ $errors->has('sales_date') ? ' is-invalid' : '' }}"
                                   aria-describedby="sales_date"
                                   placeholder="Select a Sales Date"
                            >
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <select class="form-control select2bs4 {{ $errors->has('client') ? ' is-invalid' : '' }}"
                                    id="client"
                                    name="client"
                                    style="width: 100%;"
                            ...
                            >
                                ...
                                @foreach($client as $clients)
                                    <option value="{{ $clients->id }}"
                                        {{ $sale->client_id == $clients->id ? 'selected' : '' }}
                                    >
                                        {{ $clients->name }}
                                    </option>
                                @endforeach
                ...
                <div class="row">
                    @foreach($sales_detail as $sales_details)
...
{{--                                           step="any"--}}
                                           ...
                                           name="quantities[]"
                                           value="{{ old('quantities.' . $loop->index, $sales_details->qty) }}"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text"
                                           ...
                                           name="stock"
                                           value="{{ $sales_details->stock . ' ' . $sales_details->unit_shortcut }}"
                                           disabled
                                    >
                                </div>
                            </div>
...
{{--                                           step="any"--}}
                                           ...
                                           data-number="{{ $loop->iteration }}"
                                           name="unitPrices[]"
                                           value="{{ old('unitPrices.' . $loop->index, $sales_details->unit_price) }}"
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <input type="number"
                                           ...
                                           data-number="{{ $loop->iteration }}"
                                           name="discounts[]"
                                           value="{{ old('discounts.' . $loop->index, $sales_details->single_discount) }}"
...
{{--                                   min="0"--}}
                                   ...
                                   name="singleTotal[]"
                                   value="{{ old('singleTotal.' . $loop->index, $sales_details->single_total) }}"
...
{{--                               min="0"--}}
                               id="transportCost"
                               name="transportCost"
                               value="{{ old('transportCost', $sale->transport_cost) }}"
...
{{--                               min="0"--}}
                               id="total"
                               name="total"
                               value="{{ old('total', $sale->grand_total) }}"
                ...
                <input type="hidden" value="{{ $sale->salesDetails->count() }}" id="product-count" name="totalProducts">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text"
                                   name="total_paid"
                                   value="{{ old('total_paid', $sale->total_paid) }}"
...
@push('scripts')
    ...
    <script>
        // flatpickr
        const myInput = [
            document.querySelector(".sales_date"),
        ];
    ...
    <script>
        ...
        $(".product-select").on("change", function () {
            ...
            $.ajax({
                ...
                url: '/dashboard/sales/get-stock/' + id,
            })
                .done(function(data){
